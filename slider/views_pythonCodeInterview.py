...
def getAnUniqueTitle():
    '''!!!!!!!!!!!!!!!获取唯一列，今后放到模型的save() 中，暂时在此处实现（暂时调试使用@2014年10月12日）!!!!!!!!!!!!'''
    theTitle = ""
    ...
    theTitle = "%0.8d" % random.randint(0,99999999)#所有暂时使用随机方式
...
def updateslider(request,userURL=None,sliderURL=None):
    '''修改幻灯片内容'''
    siteuser = getUserByUurl(userURL)
...
#作者
    siteuser = getUserByUurl(sliderURL)
#当前用户
    if 'uid' in request.session:
        curruser = SiteUser.objects.get(id__exact=request.session['uid'])
    else:
        curruser = 0#guest user

#浏览的幻灯片，先用短链接名称查找，没有的话使用全链接名称
    theSlider = Slider.objects.get(author=siteuser,shortTitle=sliderURL)
    if not theSlider:
        theSlider = Slider.objects.get(author=siteuser,slider_ID=sliderURL)
...
#二维码
    qrcodeHTML = ""
    minorURL = ""#需要在此处定义否则在RequestContext找不到minorURL变量
    if len(sliderPage) != 0:
        url = "%s" % request.build_absolute_uri(None)#获取当前页面url
        urlPart = url.split('/')
        urlPart[-2] = sliderURL
        minorURL = ('/'.join(urlPart))
        if not theSlider.qrcode:
            theSlider.qrcode = qrcode_datauri(minorURL)#把二维码编码为BASE64数据
            theSlider.save()#保存二维码
            
    extraInfo = {}
    if 'uid' in request.session:
        extraInfo['isSelf'] = request.session['uid'] == siteuser.id
    else:
        extraInfo['isSelf'] = False
...
def getIsfavoriteTag(request,tagname):
    '''Is current user like the named tag? '''
    try:
