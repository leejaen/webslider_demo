...
def protected(func):
    ...
    def deco(request, *args, **kwargs):
        if not test_func(request):
            return HttpResponse("<script>window.parent.upload_avatar_error('%s')</script>" % '禁止操作')
        try:
            return func(request, *args, **kwargs)
        except UploadAvatarError as e:
            return HttpResponse("<script>window.parent.upload_avatar_error('%s')</script>" % e)
    return deco
...
def upload_avatar(request):
    ...
    if uploaded_file.size > AVATAR_UPLOAD_MAX_SIZE * 1024:
        raise UploadAvatarError('图片不能大于{0}KB'.format(AVATAR_UPLOAD_MAX_SIZE))
    ...
    try:
        '''
        Thanks http://v2ex.com/t/72177
        'w' 是以文本形式打开文件， 'wb' 是二进制方式， 这个在windows上是不同的，但linux中是一样的， 所以我写的是 'w'
    ...
    return HttpResponse("<script>window.parent.upload_avatar_success('%s')</script>" % (AVATAR_UPLOAD_URL_PREFIX + new_name))
...
def crop_avatar(request):
    ...
    return HttpResponse("<script>window.parent.crop_avatar_success('%s')</script>" % '成功')
